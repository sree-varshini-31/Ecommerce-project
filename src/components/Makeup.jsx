...
export default function Makeup() {
 ...
 const navigate = useNavigate(); // added navigation
  ...
  useEffect(() => {
    setLoading(true);
    fetch("http://localhost:8080/api/products/category/makeup")
      .then(res => {
        if (!res.ok) throw new Error("Failed to fetch makeup products");
        return res.json();
      })
  ...
  return (
    <div>
      ...
      <div className="w-full mb-6">
        <img
          src="/images/makeup-banner.jpg"   // <-- your banner image path
          alt="Makeup Banner"
          className="w-full h-150 object-cover rounded-xl shadow"
        />
      </div>
      {/* <h2 className="text-2xl font-semibold p-6">Makeup</h2> */}
